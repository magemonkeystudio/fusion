    ...
    public static void migrateIntoSQL() {
        ...
        for (Map<?, ?> typeData : typesSection) {
            ...
            for (Map.Entry<CraftingTable, Integer> table : playerData.data.entrySet()) {
                SQLManager.professions().setProfession(playerData.uuid, new Profession(-1, playerData.getUuid(), table.getKey().getName(), playerData.getExperience(table.getKey()), false, false));
                //playerData.remove(table.getKey().getName());
                Fusion.getInstance().getLogger().info("Migrated " + playerData.getUuid() + " with " + table.getKey().getName());
            }
